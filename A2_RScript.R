...
ggplot(penalty_matches, aes(x = HomePenalty)) + # Create a ggplot object using the penalty_matches dataset and map HomePenalty to the x-axis
  geom_histogram(binwidth = 1, fill = "lightblue", color = "black") + # Draw a histogram
  labs(title = "Distribution of Home Penalty Goals in Penalty Shootout Matches", # Add a title for the histogram
...
ggplot(matches, aes(x = HomeTeamScore)) + # Create a ggplot object using the matches dataset and map HomeTeamScore to the x-axis
  ...
  labs(title = "Distribution of Home Team Scores across Competition Stages", # Add a title for the histogram
...
ggplot(matches_tournaments, aes(x = HomeTeamScore)) + # Create a ggplot object using the matches_tournaments dataset and map HomeTeamScore to the x-axis
  ...
  labs(title = "Distribution of Home Team Scores across FIFA World Cup Tournaments", # Add a title for the histogram
       x = "Number of Goals Scored by the Home Team", # Add label for x-axis
       y = "Number of Matches") + # Add label for y-axis
  theme_minimal() # Apply a minimal theme for a clean and simple appearance
...
matches <- matches %>%
  mutate(AwayScoreGroup = case_when( # Add a new column based on conditions applied to AwayTeamScore
    AwayTeamScore <= 1 ~ "0-1 goals", # Assign "0-1 goals" if the away team scored 0 or 1 goal
    AwayTeamScore > 1 & AwayTeamScore <= 3 ~ "2-3 goals", # Assign "2-3 goals" if the away team scored 2 or 3 goal
    AwayTeamScore > 3 ~ "4+ goals" # Assign "4+ goals" if the away team scored more than 3 goals
  ))
...
matches$AwayScoreGroup <- factor (
  ...
  levels = c("0-1 goals", "2-3 goals", "4+ goals") # Define the order of the factor levels from lowest to highest score group
)
...
ggplot(matches, aes(x = HomeTeamScore)) + # Create a ggplot object using the matches dataset and map HomeTeamScore to the x-axis
  ...
  labs(title = "Distribution of Home Team Scores by Away Team Score groups", # # Add a title for the histogram
       x = "Number of Goals Scored by the Home Team", # Add label for x-axis
       y = "Number of Matches") + # Add label for y-axis
  theme_minimal() # Apply a minimal theme for a clean and simple appearance
...
ggplot(matches, aes(x = Stage, fill = Result)) + # Create a ggplot object using the matches dataset, mapping Stage to the x-axis and Result to the fill colour
  ...
  labs(title = "Match Result Distribution by Competition Stage", # Add title for the stacked bar chart
  ...
  theme_minimal() + # Apply a minimal theme for a clean and simple appearance
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) # Rotate the x-axis labels by 30° and right-align them to improve readability.
# Discussion on findings: The distribution of match result varies across different competition stages. Home team win accounts
...
ggplot(matches, aes(x = ExtraTime, fill = Result)) + # Create a ggplot object using the matches dataset, mapping ExtraTime to the x-axis and Result to the fill colour
  geom_bar(position = "fill") + # Draw stacked bars scaled to proportions (each bar sums to 100%)
  scale_y_continuous(labels = scales::percent) + # Display the y-axis as percentage instead of proportions
  labs(title = "Match Result Distribution by Extra Time", # Add title for the stacked bar chart
